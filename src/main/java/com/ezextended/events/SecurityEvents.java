...
public class SecurityEvents {
	...
	private void securityCanceler(PlayerInteractEvent e) {
		...
		TileEntitySecure tile;
		EntityPlayer p = e.getEntityPlayer();
		
		if(b instanceof StorageMultiblock) {
			if((tile = EZUtils.findSecureBlock(new BlockRef(b, e.getPos().getX(), e.getPos().getY(), e.getPos().getZ()), w, null)) != null) {
	...
	public void onBlockPlaced(PlaceEvent e) {
		...
		for(BlockRef b : blocks) {
			if(b.block instanceof StorageMultiblock) {
				if((tile = EZUtils.findSecureBlock(new BlockRef(b.block, b.pos.getX(), b.pos.getY(), b.pos.getZ()), e.getWorld(), null)) != null) {
